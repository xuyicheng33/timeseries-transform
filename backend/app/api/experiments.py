...
from app.models import Configuration, Dataset, Experiment, Result, User, experiment_results
...
async def check_result_access(result_id: int, db: AsyncSession, user: User) -> Result:
    ...
    if result_obj.dataset_id:
        ds_result = await db.execute(select(Dataset).where(Dataset.id == result_obj.dataset_id))
...
async def get_experiment_or_404(
    experiment_id: int, db: AsyncSession, user: User, load_results: bool = False
) -> Experiment:
    """获取实验组，不存在或无权限则抛出异常"""
...
def build_experiment_response(
    experiment: Experiment, result_count: int = 0, dataset_name: str = None
) -> ExperimentResponse:
    """构建实验组响应"""
...
async def list_experiments(
    ...
    search: str | None = Query(None, description="搜索名称/描述"),
    ...
    if search:
        ...
        query = query.where((Experiment.name.ilike(search_pattern)) | (Experiment.description.ilike(search_pattern)))
        count_query = count_query.where(
            (Experiment.name.ilike(search_pattern)) | (Experiment.description.ilike(search_pattern))
        )
    ...
    result_counts_query = await db.execute(
        select(experiment_results.c.experiment_id, func.count(experiment_results.c.result_id).label("count"))
        .where(experiment_results.c.experiment_id.in_(exp_ids))
    ...
    dataset_names = {}
    if dataset_ids:
        ds_result = await db.execute(select(Dataset.id, Dataset.name).where(Dataset.id.in_(dataset_ids)))
...
async def create_experiment(
    data: ExperimentCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """创建实验组"""
    # 验证数据集（如果提供）
    dataset_name = None
    ...
    if data.result_ids:
        ...
        if data.dataset_id is None and experiment_dataset_id is not None:
            ...
            # 获取数据集名称
            ds_result = await db.execute(select(Dataset.name).where(Dataset.id == experiment_dataset_id))
...
async def get_experiment(
    ...
    if experiment.dataset_id:
        ds_result = await db.execute(select(Dataset.name).where(Dataset.id == experiment.dataset_id))
        dataset_name = ds_result.scalar()
...
async def update_experiment(
    ...
    if "dataset_id" in update_data:
        if update_data["dataset_id"]:
            ds_result = await db.execute(select(Dataset).where(Dataset.id == update_data["dataset_id"]))
            dataset = ds_result.scalar_one_or_none()
            if not dataset:
                raise HTTPException(status_code=404, detail="数据集不存在")
            # 检查数据集读取权限（遵循共享模式规则）
    ...
    count_result = await db.execute(
        select(func.count()).select_from(experiment_results).where(experiment_results.c.experiment_id == experiment.id)
    )
...
async def get_experiment_summary(
    ...
    if not results:
        return ExperimentSummary(
            experiment_id=experiment.id, experiment_name=experiment.name, result_count=0, model_names=[]
        )
    ...
    for r in results:
        if r.metrics:
            for key in metrics_data:
                if key in r.metrics:
                    metrics_data[key].append({"result_id": r.id, "value": r.metrics[key], "model_name": r.algo_name})
...
async def list_all_tags(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """获取用户所有实验组的标签列表"""
    result = await db.execute(select(Experiment.tags).where(Experiment.user_id == current_user.id))
...
async def export_experiment(
    ...
    configurations = []
    if dataset:
        cfg_result = await db.execute(select(Configuration).where(Configuration.dataset_id == dataset.id))
