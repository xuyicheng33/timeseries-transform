...
class PredictionDecompositionRequest(BaseModel):
    ...
    period: int | None = Field(default=None, description="季节性周期（自动检测如果为空）")
...
def _calculate_rolling_confidence_interval(
    residuals: np.ndarray, predictions: np.ndarray, window_size: int, confidence_level: float
) -> tuple:
    ...
    rolling_std = (
        residuals_series.rolling(window=window_size, center=True, min_periods=1)
        .std()
        .fillna(residuals_series.std())
        .values
    )
...
async def analyze_feature_importance(
    ...
    """
    # 获取结果
    result = await db.execute(select(Result).where(Result.id == request.result_id))
    ...
    features = [
        FeatureImportance(feature_name=name, importance=round(imp, 6), rank=i + 1)
...
async def generate_error_heatmap(
    request: ErrorHeatmapRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    误差热力图
    ...
    for res in results:
        ...
        try:
            ...
            valid_results.append({"result": res, "errors": errors, "length": len(errors)})
        except Exception:
            continue
    ...
    for item in valid_results:
        ...
        hist, _, _ = np.histogram2d(time_values, errors, bins=[time_bins, error_bins])
        ...
        for x in range(request.bins):
            for y in range(request.bins):
                count = int(hist[x, y])
                if count > 0:
                    cells.append(HeatmapCell(x_bin=x, y_bin=y, count=count, percentage=round(count / total * 100, 2)))
    ...
    return ErrorHeatmapResponse(heatmaps=heatmaps, unified_error_range=[error_min, error_max])
...
async def decompose_prediction(
    ...
    components.append(
        DecompositionComponent(
            name="trend", values=_downsample_uniform(trend.tolist(), 2000), indices=_downsample_uniform(indices, 2000)
        )
    )
    ...
    return PredictionDecompositionResponse(
        result_id=result_obj.id, result_name=result_obj.name, components=components, detected_period=detected_period
    )
