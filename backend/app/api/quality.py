...
from app.services.cleaning import apply_cleaning, preview_cleaning
...
def _read_csv_with_sampling(
    filepath: str, encoding: str = "utf-8", max_rows: int = None, sample_size: int = None
...
async def get_quality_report(
    ...
    # 生成质量报告
    try:
        report = await run_in_executor(analyze_data_quality, df, dataset.id, dataset.name, request)
...
async def apply_data_cleaning(
    ...
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="数据清洗操作仅限管理员执行")
...
async def get_outlier_details(
    ...
    for idx in stat.outlier_indices[:100]:  # 最多返回100个
        if idx < len(df):
            outlier_data.append(
                {"index": idx, "value": float(series.iloc[idx]) if pd.notna(series.iloc[idx]) else None}
            )
    ...
    return {
        ...
        "stats": {"min": stat.min_value, "max": stat.max_value, "mean": stat.mean_value, "std": stat.std_value},
    }
