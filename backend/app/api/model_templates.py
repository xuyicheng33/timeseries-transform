...
async def init_preset_templates(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """
    初始化预置模型模板（仅管理员可用）
    如果模板已存在则跳过
    """
    ...
    return {"message": "预置模板初始化完成", "created": created_count, "skipped": skipped_count}


@router.post("", response_model=ModelTemplateResponse)
async def create_template(
    data: ModelTemplateCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """创建模型模板"""
    template = ModelTemplate(
        name=data.name,
...
async def list_templates(
    ...
    if search:
        conditions.append(or_(ModelTemplate.name.ilike(f"%{search}%"), ModelTemplate.description.ilike(f"%{search}%")))
...
async def list_all_templates(
    category: str = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """获取所有模型模板（不分页，用于下拉选择）"""
    conditions = [
        or_(ModelTemplate.is_system == True, ModelTemplate.is_public == True, ModelTemplate.user_id == current_user.id)
    ...
    query = select(ModelTemplate).order_by(ModelTemplate.is_system.desc(), ModelTemplate.name.asc())
    for cond in conditions:
        query = query.where(cond)
...
async def list_categories(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    categories = [{"value": row[0], "label": row[0], "count": row[1]} for row in result.all()]
...
async def get_template(
    template_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """获取模型模板详情"""
    result = await db.execute(select(ModelTemplate).where(ModelTemplate.id == template_id))
