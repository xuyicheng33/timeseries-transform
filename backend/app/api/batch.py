...
from app.services.permissions import ResourceType, check_owner_or_admin, check_read_access
...
async def batch_delete_datasets(
    ...
    return BatchDeleteResult(
        success_count=success_count, failed_count=len(failed_ids), failed_ids=failed_ids, errors=errors[:10]
    )


@router.post("/configurations/delete", response_model=BatchDeleteResult)
async def batch_delete_configurations(
    request: BatchDeleteRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    批量删除配置（所有者或管理员）
    """
...
async def export_data(
    ...
    if request.include_configs:
        for ds in datasets:
            cfg_result = await db.execute(select(Configuration).where(Configuration.dataset_id == ds.id))
    ...
    if request.include_results:
        for ds in datasets:
            res_result = await db.execute(select(Result).where(Result.dataset_id == ds.id))
    ...
    try:
        zip_path = await run_in_executor(
            _create_export_zip, datasets, configs, results, request.include_results, temp_dir, skipped_files
        )
...
def _parse_import_zip(zip_path: str) -> dict:
    """解析导入的 ZIP 文件（同步）"""
    data = {"metadata": None, "datasets": [], "configurations": [], "results": [], "files": {}}
...
async def preview_import(file: UploadFile = File(...), current_user: User = Depends(get_admin_user)):  # 仅管理员可用
    """
    预览导入内容（仅管理员）
