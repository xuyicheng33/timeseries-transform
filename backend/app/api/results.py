...
from app.schemas import PaginatedResponse, ResultResponse, ResultUpdate
...
async def upload_result(
    ...
    if not has_predicted_value:
        await run_in_executor(safe_rmtree, str(result_dir))
        await db.rollback()
        raise HTTPException(status_code=400, detail="缺少必需列: predicted_value。文件必须包含 'predicted_value' 列")
    ...
    if not has_true_value:
        ...
        if target_column not in dataset_df.columns:
            ...
            await db.rollback()
            raise HTTPException(
                status_code=400, detail=f"数据集中不存在列 '{target_column}'。可用列: {list(dataset_df.columns)}"
            )
        ...
        if len(df) != len(dataset_df):
            ...
            await db.rollback()
            raise HTTPException(
                status_code=400, detail=f"预测结果行数 ({len(df)}) 与数据集行数 ({len(dataset_df)}) 不匹配"
            )
    ...
    try:
        ...
        # 验证数据，拒绝包含 NaN 的数据
        true_vals, pred_vals, _ = validate_numeric_data(true_vals, pred_vals, strategy=NaNHandlingStrategy.REJECT)
...
async def preview_result(
    result_id: int, rows: int = 100, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """预览结果数据（前 N 行）"""
    # 限制最大行数
