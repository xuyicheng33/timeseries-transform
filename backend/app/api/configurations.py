...
async def create_configuration(
    data: ConfigurationCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """
    创建配置
    ...
    filename = generate_standard_filename(
        ...
        anomaly_type=(
            data.anomaly_type.value
            if data.anomaly_type and hasattr(data.anomaly_type, "value")
            else (data.anomaly_type or "")
        ),
        injection_algorithm=(
            data.injection_algorithm.value
            if data.injection_algorithm and hasattr(data.injection_algorithm, "value")
            else (data.injection_algorithm or "")
        ),
        sequence_logic=(
            data.sequence_logic.value
            if data.sequence_logic and hasattr(data.sequence_logic, "value")
            else (data.sequence_logic or "")
        ),
...
async def list_all_configurations(
    dataset_id: int | None = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
...
async def get_configuration(
    config_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """获取配置详情"""
...
async def generate_filename_api(data: GenerateFilenameRequest, current_user: User = Depends(get_current_user)):
    """生成标准文件名"""
    filename = generate_standard_filename(
