...
async def get_correlation_matrix(
    dataset_id: int,
    columns: Optional[str] = Query(default=None, description="逗号分隔的列名，为空则使用所有数值列"),
...
async def get_trend_analysis(
    dataset_id: int,
    column: str,
    time_column: Optional[str] = Query(default=None, description="时间列名"),
...
def _compute_trend(x: np.ndarray, y: np.ndarray, window: int, max_points: int) -> dict:
    ...
    return {
        ...
        "moving_avg": [
            [float(x_sampled[i]), float(ma_padded[i])] 
            for i in range(len(y_sampled)) 
            if not np.isnan(ma_padded[i])
        ],
...
def _compute_overview(df: pd.DataFrame, dataset_name: str) -> dict:
    ...
    return {
        ...
        "categorical_columns": [
            c["name"] for c in column_summary 
