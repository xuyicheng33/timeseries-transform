...
from app.models import User, Dataset, Result, Configuration
...
def check_write_access(
    ...
    if parent_dataset is not None:
        raise HTTPException(
            status_code=403, 
            detail=f"无权{action}此{resource_type}，只有{resource_type}所有者或数据集所有者可以操作"
        )
    else:
        raise HTTPException(
            status_code=403, 
            detail=f"无权{action}此{resource_type}，只有{resource_type}所有者可以操作"
        )
...
def check_dataset_write_access(dataset: Dataset, user: User, action: str = "写入") -> None:
    ...
    raise HTTPException(
        status_code=403, 
        detail=f"无权向此数据集{action}，只有数据集所有者可以操作"
    )
...
def get_isolation_conditions(user: User, model_class: Type[T]) -> Tuple[List[Any], bool]:
    ...
    if model_class == Dataset:
        conditions.append(
            or_(Dataset.user_id == user.id, Dataset.is_public == True)
        )
...
def can_access_result(result: Result, dataset: Optional[Dataset], user: User) -> bool:
    """
