...
from app.schemas.enums import MissingStrategy, OutlierAction
...
class DataCleaner:
    ...
    def _record_change(self, index: int, column: str, original_value: Any, new_value: Any, action: str):
        """记录变更"""
        # 限制记录数量
        if len(self.changes) < 100:
            ...
            self.changes.append(
                CleaningPreviewRow(
                    index=int(index), column=column, original_value=original_value, new_value=new_value, action=action
                )
            )
    ...
    def handle_outliers(self, config: CleaningConfig) -> int:
        ...
        for col in target_columns:
            ...
            lower_bound, upper_bound = self._get_outlier_bounds(series, config.outlier_method, config.outlier_params)
    ...
    def preview(self, config: CleaningConfig) -> CleaningPreviewResponse:
        ...
        _, original_outlier_ratio = original_analyzer.detect_outliers(
            method=config.outlier_method, params=config.outlier_params
        )[1:3]
