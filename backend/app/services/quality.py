...
from app.schemas.enums import ColumnDataType, OutlierMethod, QualityLevel
...
class QualityAnalyzer:
    ...
    def detect_outliers(
        self, method: str = "iqr", params: dict[str, Any] = None, columns: list[str] | None = None
    ...
    def _get_outlier_bounds(self, series: pd.Series, method: str, params: dict[str, Any]) -> tuple[float, float]:
        """
        根据方法计算异常值边界
    ...
    def analyze_time_series(self) -> TimeSeriesAnalysis | None:
        """分析时序特征"""
    ...
    def generate_suggestions(
        ...
        duplicate_count: int,
        time_analysis: TimeSeriesAnalysis | None,
    ...
    def generate_report(self, request: QualityCheckRequest = None) -> DataQualityReport:
        ...
        outlier_stats, total_outliers, total_outlier_ratio = self.detect_outliers(
            method=request.outlier_method, params=request.outlier_params, columns=request.check_columns
        )
        ...
        quality_score, quality_level = self.calculate_quality_score(
            total_missing_ratio, total_outlier_ratio, duplicate_ratio, time_analysis
        )

        # 生成建议
        suggestions = []
        if request.include_suggestions:
            suggestions = self.generate_suggestions(missing_stats, outlier_stats, duplicate_count, time_analysis)
...
def analyze_data_quality(
    df: pd.DataFrame, dataset_id: int, dataset_name: str, request: QualityCheckRequest = None
) -> DataQualityReport:
    """
    分析数据质量的便捷函数
