...
from app.models import User, Dataset, Result
...
class TestResultList:
    ...
    async def test_list_results_with_data(self, client: AsyncClient, auth_headers: dict, test_result: Result):
        ...
        assert data["items"][0]["algo_name"] == "TestModel"
    
    @pytest.mark.asyncio
    async def test_list_results_filter_by_dataset(self, client: AsyncClient, auth_headers: dict, test_result: Result, test_dataset: Dataset):
        """测试按数据集筛选"""
        response = await client.get(
            f"/api/results?dataset_id={test_dataset.id}",
...
class TestResultGet:
    ...
    async def test_get_result_success(self, client: AsyncClient, auth_headers: dict, test_result: Result):
        ...
        assert data["algo_name"] == "TestModel"
...
class TestResultUpload:
    ...
    @pytest.mark.asyncio
    async def test_upload_result_success(self, client: AsyncClient, auth_headers: dict, test_dataset: Dataset, temp_result_csv: str):
        """测试上传结果"""
        ...
        assert data["algo_name"] == "UploadedModel"
        assert data["algo_version"] == "1.0.0"
        # 应该自动计算指标
    ...
    @pytest.mark.asyncio
    async def test_upload_result_missing_columns(self, client: AsyncClient, auth_headers: dict, test_dataset: Dataset, temp_csv_file: str):
        """测试缺少必需列"""
        ...
        assert "true_value" in response.json()["detail"].lower() or "predicted_value" in response.json()["detail"].lower()
...
class TestResultUpdate:
    ...
    async def test_update_result_success(self, client: AsyncClient, auth_headers: dict, test_result: Result):
        ...
        assert data["algo_name"] == "UpdatedModel"
...
class TestResultModelNames:
    ...
    @pytest.mark.asyncio
    async def test_get_model_names_by_dataset(self, client: AsyncClient, auth_headers: dict, test_result: Result, test_dataset: Dataset):
        """测试按数据集获取模型名称"""
        response = await client.get(
            f"/api/results/model-names?dataset_id={test_dataset.id}",
            headers=auth_headers
        )
