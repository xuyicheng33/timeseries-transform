...
class TestSamplingFunction:
    ...
    def test_sampling_reproducible(self, large_csv_file: str):
        """测试采样结果可重复"""
        df1, _, _ = _read_csv_with_sampling(
            large_csv_file,
            encoding='utf-8',
            max_rows=100,
            sample_size=200
        ...
        df2, _, _ = _read_csv_with_sampling(
            large_csv_file,
            encoding='utf-8',
            max_rows=100,
            sample_size=200
...
class TestQualityReportAPI:
    ...
    @pytest.mark.asyncio
    async def test_quality_report_success(
        self, 
        client: AsyncClient, 
        ...
        response = await client.get(
            f"/api/quality/{dataset_id}/report",
            headers=admin_auth_headers
        )
