"""
按 stationid 分组合并 station_baseinfo 数据集
...
# 配置
SOURCE_DIR = Path(r"E:\待处理数据\station_baseinfo")
OUTPUT_DIR = SOURCE_DIR / "station_baseinfo_merged"
...
def merge_by_stationid():
    """按 stationid 分组合并数据"""
    print("="*80)
    print("按 stationid 分组合并 station_baseinfo 数据集")
    print("="*80)
    ...
    for idx, stationid in enumerate(sorted(stationid_data.keys()), 1):
        ...
        # 保存文件
        output_file = OUTPUT_DIR / f"station_baseinfo_stationid{stationid}.csv"
        # 确保时间格式完整保存（包含时分秒）
