"""
合并 powerforecast_fd_weatherdata 数据集
去除元数据列，只保留气象相关字段
"""
import pandas as pd
import glob
...
OUTPUT_FILE = OUTPUT_DIR / "powerforecast_weatherdata_station41_complete.csv"

# 去除元数据列，只保留气象字段
EXCLUDE_COLUMNS = ['id', 'savetime', 'filename', 'stationid']
TARGET_STATION_ID = 41


def merge_weatherdata():
    """合并所有 powerforecast_fd_weatherdata 文件，去除元数据列"""
    
    print("=" * 60)
    print("开始合并 powerforecast_fd_weatherdata 数据集")
    print("=" * 60)
    print(f"源目录: {SOURCE_DIR}")
    print(f"目标站点: stationid={TARGET_STATION_ID}")
    print(f"排除列: {', '.join(EXCLUDE_COLUMNS)}")
    print()
    
    # 查找所有文件
    pattern = str(SOURCE_DIR / "powerforecast_fd_weatherdata_*.csv")
    ...
    for i, file in enumerate(files, 1):
        ...
        try:
            ...
            # 检查必需列是否存在
            if 'stationid' not in df.columns or 'datatime' not in df.columns:
            ...
            if len(df_filtered) > 0:
                # 去除元数据列
                cols_to_keep = [col for col in df_filtered.columns if col not in EXCLUDE_COLUMNS]
    ...
    # 去重（保留最后一条记录）
    print("\n检查重复数据...")
    duplicates = merged_df.duplicated(subset=['datatime'], keep='last').sum()
    if duplicates > 0:
        print(f"⚠️  发现 {duplicates:,} 条重复记录，正在删除...")
        merged_df = merged_df.drop_duplicates(subset=['datatime'], keep='last')
    ...
    print("数据质量报告")
    print("=" * 60)
    print(f"时间范围: {merged_df['datatime'].min()} 至 {merged_df['datatime'].max()}")
    print(f"总记录数: {len(merged_df):,} 行")
    print(f"\n保留的列 ({len(merged_df.columns)} 列):")
